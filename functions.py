...
class Error(Exception):
    pass
class LengthsError(Error):
    pass
class MyTypeError(Error):
    pass
def validate_entry(entry: list):
    try:
        if (len(entry[1]) == len(entry[3]) and (len(entry[5]) == len(entry[3]) or not len(entry[5]))) and (len(entry[2]) == len(entry[4]) and (len(entry[6]) == len(entry[4]) or not len(entry[6]))):
            if all(entry[i][j].isdigit() for i in [3, 4] for j in range(len(entry[i]))):
                return 1, entry
            else:
                raise MyTypeError("Debit or credit amounts were not integer values.")
        else:
            raise LengthsError("Ensure debit (credit) accounts, amounts and folios have equal lengths.")
    except MyTypeError as e:
        return e
    except LengthsError as e:
        return e

def add_entry(project_name):
    response = validate_entry(input_entry())
    if isinstance(response, tuple):
        journals_data = read_journals(project_name)
        journals_data.append(response[1])
        print("The following entry was recorded:")
        print_entry(response[1])
        write_journals(project_name, journals_data)
    else:
        print(f"Error encountered.\n[-] {response}")

def edit_entry(project_name):
    edit_number = int(input("Enter the serial number of that entry (starting with 1; remember that years also count as entries): "))
    edit_number -= 1 # indexify
    eech = input("Should the replacement be an year entry or normal entry? y/n: ").lower().strip()
    print("Input values for the new entry:")
    journals_data = read_journals(project_name)
    if eech == "y":
        year = input("Enter the year to replace with: ")
        updated = [f"{year}"]
    elif eech == "n":
        response = validate_entry(input_entry())
        if isinstance(response, tuple):
    ...
    if updated != 0:
        try:
            journals_data[edit_number] = updated
            write_journals(project_name, journals_data)
            print("The desired change was made. Updated entry stands:")
            if eech == "y": print("Year:", updated[0])
            else: print_entry(updated)
